...
namespace QTrading::Infra::Exchanges::BinanceSim::Calibration {
...
MetricVector average_slice(
    const std::vector<MetricVector>& series,
    size_t begin,
    size_t end)
{
    MetricVector out{};
    if (begin >= end || begin >= series.size()) {
        return out;
    }
    const size_t clamped_end = std::min(end, series.size());
    ...
    for (size_t i = begin; i < clamped_end; ++i) {
        out.fill_ratio += series[i].fill_ratio;
        out.avg_slippage_bps += series[i].avg_slippage_bps;
        out.taker_ratio += series[i].taker_ratio;
        out.funding_cost += series[i].funding_cost;
        out.tail_loss += series[i].tail_loss;
    }

    const double denom = static_cast<double>(count);
    out.fill_ratio /= denom;
    out.avg_slippage_bps /= denom;
    out.taker_ratio /= denom;
    out.funding_cost /= denom;
    out.tail_loss /= denom;
    return out;
}
