#include "Exchanges/BinanceSimulator/BinanceExchange.hpp"
#include "Exchanges/BinanceSimulator/Domain/AsyncOrderLatencyScheduler.hpp"
#include "Exchanges/BinanceSimulator/Domain/BinanceRejectSurface.hpp"
#include "Exchanges/BinanceSimulator/Domain/OrderEntryService.hpp"

using QTrading::Dto::Trading::OrderSide;

namespace QTrading::Infra::Exchanges::BinanceSim {

bool BinanceExchange::SpotApi::place_order(const std::string& symbol,
    double quantity,
    double price,
    OrderSide side,
    bool reduce_only,
    const std::string& client_order_id,
    Account::SelfTradePreventionMode stp_mode)
{
    std::lock_guard<std::mutex> lk(owner_.account_mtx_);
    if (Domain::AsyncOrderLatencyScheduler::TrySchedule(
        owner_.order_latency_bars_,
        owner_.processed_steps_,
        owner_.next_async_order_request_id_,
        [&owner = owner_, &symbol, quantity, price, side, reduce_only, &client_order_id, stp_mode](
            const Domain::AsyncOrderLatencyScheduler::ScheduleTicket& ticket) {
            owner.push_async_order_ack_locked_(BinanceExchange::AsyncOrderAck{
                ticket.request_id,
                BinanceExchange::AsyncOrderAck::Status::Pending,
                QTrading::Dto::Trading::InstrumentType::Spot,
                symbol,
                quantity,
                price,
                side,
                QTrading::Dto::Trading::PositionSide::Both,
                reduce_only,
                ticket.submitted_step,
                ticket.due_step,
                0,
                Account::OrderRejectInfo::Code::None,
                {},
                client_order_id,
                stp_mode
            });
        },
        [&owner = owner_, &symbol, quantity, price, side, reduce_only, &client_order_id, stp_mode](
            const Domain::AsyncOrderLatencyScheduler::ScheduleTicket& ticket) {
            const uint64_t req_id = ticket.request_id;
            const uint64_t submitted_step = ticket.submitted_step;
            const uint64_t due = ticket.due_step;
            owner.enqueue_deferred_order_locked_(due, [owner_ptr = &owner, req_id, submitted_step, due, symbol, quantity, price, side, reduce_only, client_order_id, stp_mode](Account& acc, uint64_t step_seq) {
                const bool ok = acc.spot.place_order(symbol, quantity, price, side, reduce_only, client_order_id, stp_mode);
                auto rej = acc.consume_last_order_reject_info();
                auto binance_reject = Domain::BinanceRejectSurface::MapToBinanceError(rej);
                owner_ptr->push_async_order_ack_locked_(BinanceExchange::AsyncOrderAck{
                    req_id,
                    ok ? BinanceExchange::AsyncOrderAck::Status::Accepted : BinanceExchange::AsyncOrderAck::Status::Rejected,
                    QTrading::Dto::Trading::InstrumentType::Spot,
                    symbol,
                    quantity,
                    price,
                    side,
                    QTrading::Dto::Trading::PositionSide::Both,
                    reduce_only,
                    submitted_step,
                    due,
                    step_seq,
                    rej ? rej->code : Account::OrderRejectInfo::Code::None,
                    rej ? rej->message : std::string{},
                    client_order_id,
                    stp_mode,
                    binance_reject.first,
                    std::move(binance_reject.second)
                });
            });
        })
        .has_value()) {
        return true;
    }
    return Domain::OrderEntryService::PlaceSpotLimitSync(
        *owner_.account_engine_, symbol, quantity, price, side, reduce_only, client_order_id, stp_mode);
}

bool BinanceExchange::SpotApi::place_order(const std::string& symbol,
    double quantity,
    OrderSide side,
    bool reduce_only,
    const std::string& client_order_id,
    Account::SelfTradePreventionMode stp_mode)
{
    std::lock_guard<std::mutex> lk(owner_.account_mtx_);
    if (Domain::AsyncOrderLatencyScheduler::TrySchedule(
        owner_.order_latency_bars_,
        owner_.processed_steps_,
        owner_.next_async_order_request_id_,
        [&owner = owner_, &symbol, quantity, side, reduce_only, &client_order_id, stp_mode](
            const Domain::AsyncOrderLatencyScheduler::ScheduleTicket& ticket) {
            owner.push_async_order_ack_locked_(BinanceExchange::AsyncOrderAck{
                ticket.request_id,
                BinanceExchange::AsyncOrderAck::Status::Pending,
                QTrading::Dto::Trading::InstrumentType::Spot,
                symbol,
                quantity,
                0.0,
                side,
                QTrading::Dto::Trading::PositionSide::Both,
                reduce_only,
                ticket.submitted_step,
                ticket.due_step,
                0,
                Account::OrderRejectInfo::Code::None,
                {},
                client_order_id,
                stp_mode
            });
        },
        [&owner = owner_, &symbol, quantity, side, reduce_only, &client_order_id, stp_mode](
            const Domain::AsyncOrderLatencyScheduler::ScheduleTicket& ticket) {
            const uint64_t req_id = ticket.request_id;
            const uint64_t submitted_step = ticket.submitted_step;
            const uint64_t due = ticket.due_step;
            owner.enqueue_deferred_order_locked_(due, [owner_ptr = &owner, req_id, submitted_step, due, symbol, quantity, side, reduce_only, client_order_id, stp_mode](Account& acc, uint64_t step_seq) {
                const bool ok = acc.spot.place_order(symbol, quantity, side, reduce_only, client_order_id, stp_mode);
                auto rej = acc.consume_last_order_reject_info();
                auto binance_reject = Domain::BinanceRejectSurface::MapToBinanceError(rej);
                owner_ptr->push_async_order_ack_locked_(BinanceExchange::AsyncOrderAck{
                    req_id,
                    ok ? BinanceExchange::AsyncOrderAck::Status::Accepted : BinanceExchange::AsyncOrderAck::Status::Rejected,
                    QTrading::Dto::Trading::InstrumentType::Spot,
                    symbol,
                    quantity,
                    0.0,
                    side,
                    QTrading::Dto::Trading::PositionSide::Both,
                    reduce_only,
                    submitted_step,
                    due,
                    step_seq,
                    rej ? rej->code : Account::OrderRejectInfo::Code::None,
                    rej ? rej->message : std::string{},
                    client_order_id,
                    stp_mode,
                    binance_reject.first,
                    std::move(binance_reject.second)
                });
            });
        })
        .has_value()) {
        return true;
    }
    return Domain::OrderEntryService::PlaceSpotMarketSync(
        *owner_.account_engine_, symbol, quantity, side, reduce_only, client_order_id, stp_mode);
}

bool BinanceExchange::SpotApi::place_market_order_quote(const std::string& symbol,
    double quote_order_qty,
    OrderSide side,
    bool reduce_only,
    const std::string& client_order_id,
    Account::SelfTradePreventionMode stp_mode)
{
    std::lock_guard<std::mutex> lk(owner_.account_mtx_);
    if (Domain::AsyncOrderLatencyScheduler::TrySchedule(
        owner_.order_latency_bars_,
        owner_.processed_steps_,
        owner_.next_async_order_request_id_,
        [&owner = owner_, &symbol, quote_order_qty, side, reduce_only, &client_order_id, stp_mode](
            const Domain::AsyncOrderLatencyScheduler::ScheduleTicket& ticket) {
            owner.push_async_order_ack_locked_(BinanceExchange::AsyncOrderAck{
                ticket.request_id,
                BinanceExchange::AsyncOrderAck::Status::Pending,
                QTrading::Dto::Trading::InstrumentType::Spot,
                symbol,
                quote_order_qty,
                0.0,
                side,
                QTrading::Dto::Trading::PositionSide::Both,
                reduce_only,
                ticket.submitted_step,
                ticket.due_step,
                0,
                Account::OrderRejectInfo::Code::None,
                {},
                client_order_id,
                stp_mode
            });
        },
        [&owner = owner_, &symbol, quote_order_qty, side, reduce_only, &client_order_id, stp_mode](
            const Domain::AsyncOrderLatencyScheduler::ScheduleTicket& ticket) {
            const uint64_t req_id = ticket.request_id;
            const uint64_t submitted_step = ticket.submitted_step;
            const uint64_t due = ticket.due_step;
            owner.enqueue_deferred_order_locked_(due, [owner_ptr = &owner, req_id, submitted_step, due, symbol, quote_order_qty, side, reduce_only, client_order_id, stp_mode](Account& acc, uint64_t step_seq) {
                const bool ok = acc.spot.place_market_order_quote(symbol, quote_order_qty, side, reduce_only, client_order_id, stp_mode);
                auto rej = acc.consume_last_order_reject_info();
                auto binance_reject = Domain::BinanceRejectSurface::MapToBinanceError(rej);
                owner_ptr->push_async_order_ack_locked_(BinanceExchange::AsyncOrderAck{
                    req_id,
                    ok ? BinanceExchange::AsyncOrderAck::Status::Accepted : BinanceExchange::AsyncOrderAck::Status::Rejected,
                    QTrading::Dto::Trading::InstrumentType::Spot,
                    symbol,
                    quote_order_qty,
                    0.0,
                    side,
                    QTrading::Dto::Trading::PositionSide::Both,
                    reduce_only,
                    submitted_step,
                    due,
                    step_seq,
                    rej ? rej->code : Account::OrderRejectInfo::Code::None,
                    rej ? rej->message : std::string{},
                    client_order_id,
                    stp_mode,
                    binance_reject.first,
                    std::move(binance_reject.second)
                });
            });
        })
        .has_value()) {
        return true;
    }
    return Domain::OrderEntryService::PlaceSpotMarketQuoteSync(
        *owner_.account_engine_, symbol, quote_order_qty, side, reduce_only, client_order_id, stp_mode);
}

void BinanceExchange::SpotApi::close_position(const std::string& symbol, double price)
{
    std::lock_guard<std::mutex> lk(owner_.account_mtx_);
    owner_.account_engine_->spot.close_position(symbol, price);
}

void BinanceExchange::SpotApi::cancel_open_orders(const std::string& symbol)
{
    std::lock_guard<std::mutex> lk(owner_.account_mtx_);
    owner_.account_engine_->spot.cancel_open_orders(symbol);
}

} // namespace QTrading::Infra::Exchanges::BinanceSim
