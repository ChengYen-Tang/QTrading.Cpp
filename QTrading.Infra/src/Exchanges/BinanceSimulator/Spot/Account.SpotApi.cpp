    ...
    bool reduce_only)
{
    ...
    return owner_->place_order(symbol, quantity, price, side, PositionSide::Both, reduce_only);
}
...
{
    owner_->set_instrument_type(symbol, InstrumentType::Spot);
    return owner_->place_order(symbol, quantity, side, PositionSide::Both, reduce_only);
}
